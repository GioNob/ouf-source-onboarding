...
public class ManagedFormatsPublisherFixture {
  ...
  private final SemanticGapService gaps;
  private final Map<String, Map<String,Object>> configurations = new HashMap<>();
  public ManagedFormatsPublisherFixture(OnboardingService service, ManagedFileService files, CanonicalHash hashes,SemanticGapService gaps) {
    this.service=service; this.files=files; this.hashes=hashes;this.gaps=gaps;
  }
  ...
  @SuppressWarnings("unchecked") private Map<String,Object> publish(String kind) {
    try {
      ...
      if(reload) {
        ...
        String geometry=gpkg?"geom":null;
        ...
        if(gpkg){fields=new ArrayList<>(fields);fields.add(new ManagedFileService.FieldDecision("fid","EXCLUDE","OPEN",null,null,null,null,null));}
        ...
        var draft=files.onboard(asset,profile,source,source,"platform",type,List.of("core@"+binding.get("semanticVersion")),
            spatial||child?List.of("ID"):List.of("DISTRICT","CODE"),fields,spatial?"assets":child?"Children":"Assets",actor(),"r2f");
        ...
        execution.put("acquisitionMode",gpkg?"INTERNAL_MANAGED_GEOPACKAGE":shape?"INTERNAL_MANAGED_SHAPEFILE":"INTERNAL_MANAGED_ACCESS");
        execution.put("adapterId",gpkg?"managed-geopackage-v1":shape?"managed-shapefile-v1":"managed-access-v1");execution.put("adapterRuntimeVersion","1.0.0");
        ...
        if(spatial)for(String key:List.of("sourceCrs","geometryColumn"))execution.put(key,runtime.get(key));if(shape)execution.put("encoding",runtime.get("encoding"));
        ...
        version=(UUID)draft.get("onboarding_version_id");
        if(!spatial){
          var proposals=gaps.proposeAccessSchema(source,version,new OnboardingService.Actor("fixture-human","HUMAN_USER",Set.of("ouf.source-onboarding.semantic-gap.create")));
          for(var proposal:proposals){
            if(!String.valueOf(proposal.get("source_evidence")).contains("RELATIONSHIP"))continue;
            UUID gap=(UUID)proposal.get("gap_id");gaps.requestSearch(source,version,gap,"semantic-reference://"+binding.get("publicationSetId"));
            var reported=gaps.recordCandidates(source,version,gap,List.of(Map.of("semanticRef","core@"+binding.get("semanticVersion"),"status","PUBLISHED","evidence",Map.of("binding",binding,"predicate",iri("belongsTo"),"domain",iri("Child"),"range",iri("Asset"),"direction","REFERENCING_TO_REFERENCED"))),new OnboardingService.Actor("fixture-semantic","SERVICE",Set.of("ouf.semantic.discovery.result.report")));
            UUID candidate=(UUID)((Map<?,?>)((List<?>)reported.get("candidates")).getFirst()).get("candidate_id");gaps.select(source,version,gap,candidate);
          }
        }
        service.patchVersion(source,version,0,config,actor(),"r2f");activate(source,version,1);
  ...
  @SuppressWarnings("unchecked") private Map<String,Object> conflicts(){
    String source="r2f-assets";var config=new LinkedHashMap<>(configurations.get(source));
    var ex=copy(config,"extractionProfile");var runtime=copy(ex,"runtime");var udp=copy(runtime,"udp");var materialization=copy(udp,"materialization");
    materialization.put("policyRef","policy://r2f-authority/2");
    materialization.put("properties",((List<Map<String,Object>>)materialization.get("properties")).stream().map(p->{var rule=new LinkedHashMap<>(p);rule.put("authorityOrder",List.of());return rule;}).toList());
    udp.put("materialization",materialization);runtime.put("udp",udp);ex.put("runtime",runtime);config.put("extractionProfile",ex);
    var bundle=copy(config,"bundle");bundle.put("bundleVersion","2");config.put("bundle",bundle);
    UUID version=(UUID)service.createVersion(source,config,actor(),"r2f-conflict").get("onboarding_version_id");activate(source,version,0);return Map.of("status","ACTIVE","version",version);
  }
