...
class DiscoveryRuntimeTest {
  ...
  @Test void schemaOnlyDiscoveryRejectsInstanceMaterial(){UUID id=(UUID)discovery.start("world-bank","population","schema-only",Map.of("mode","SCHEMA_ONLY")).get("discovery_run_id");discovery.claim("worker",Duration.ofSeconds(30)).orElseThrow();Map<String,Object> unsafe=new LinkedHashMap<>(snapshot());unsafe.put("samples",List.of(Map.of("personName","secret")));assertThatThrownBy(()->discovery.complete(id,"worker",unsafe)).hasMessageContaining("SCHEMA_ONLY");}
