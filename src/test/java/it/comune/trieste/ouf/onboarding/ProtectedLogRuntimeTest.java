...
class ProtectedLogRuntimeTest {
  ...
  @Test @SuppressWarnings("unchecked") void secretsAreAlwaysRemovedEvenWithPiiGrant(){ProtectedLogRedactionService redaction=new ProtectedLogRedactionService();Map<String,Object> out=redaction.redact(Map.of("name","Mario","password","bad","nested",Map.of("access_token","bad","ok","yes")),true);assertThat(out).containsEntry("name","Mario").doesNotContainKey("password");assertThat((Map<String,Object>)out.get("nested")).containsEntry("ok","yes").doesNotContainKey("access_token");}
