...
class TrustedActorResolverTest {
  ...
  @Test void derivesAgentAndAuthorizationOwnedCapabilitiesFromTrustedContext(){MockHttpServletRequest request=new MockHttpServletRequest();request.setUserPrincipal((Principal)()->"agent:mcp");request.addUserRole("OUF_AI_AGENT");request.setAttribute("ouf.authorizedCapabilities",java.util.List.of("ouf.source-onboarding.submit"));var actor=resolver.actor(request);assertThat(actor.subject()).isEqualTo("agent:mcp");assertThat(actor.type()).isEqualTo("AI_AGENT");assertThat(actor.capabilities()).containsExactly("ouf.source-onboarding.submit");}
