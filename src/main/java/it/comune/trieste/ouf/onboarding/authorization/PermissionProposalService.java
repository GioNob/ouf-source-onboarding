...
public class PermissionProposalService {
 ...
 @Transactional public Receipt propose(PrincipalContext p,Change change,String idempotency){
  ...
  if(roleChange(change))db.sql("update ouf_authorization.permission_proposal set before_role_catalogue=cast(:before as jsonb) where proposal_id=:id").param("before",encode(roles.current(p.tenantId()))).param("id",id).update();
