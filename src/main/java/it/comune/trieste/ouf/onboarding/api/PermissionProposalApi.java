...
public class PermissionProposalApi {
 ...
 public record Query(String subjectId,String externalRoleRef,Integer limit,String after,String policyRef,String view){}
 ...
 @PostMapping("/read") Object read(@RequestBody byte[] raw,HttpServletRequest r){var p=caller(r,raw,PermissionProposalService.READ).principal();var q=body(raw,Query.class);if("ROLES".equals(q.view())){if(q.subjectId()!=null||q.externalRoleRef()!=null||q.limit()!=null||q.after()!=null||q.policyRef()!=null)throw new IllegalArgumentException("role view does not accept grant filters");return service.roleCatalogue(p);}if(q.view()!=null&&!"GRANTS".equals(q.view()))throw new IllegalArgumentException("invalid view");return service.access(p,q.subjectId(),q.externalRoleRef(),q.limit()==null?100:q.limit(),q.after(),q.policyRef());}
