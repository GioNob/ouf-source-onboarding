...
public class PermissionProposalApi {
 ...
 @PostMapping("/read") Object read(@RequestBody byte[] raw,HttpServletRequest r){var p=caller(r,raw,PermissionProposalService.READ).principal();var q=body(raw,Query.class);if("ROLES".equals(q.view()))return service.roleCatalogue(p);if(q.view()!=null&&!"GRANTS".equals(q.view()))throw new IllegalArgumentException("invalid view");return service.access(p,q.subjectId(),q.externalRoleRef(),q.limit()==null?100:q.limit(),q.after(),q.policyRef());}
