...
public class ManagedFileApi {
  ...
  @PostMapping("/{assetId}/profile") ResponseEntity<Map<String,Object>> requestProfile(@PathVariable UUID assetId,@RequestHeader("Idempotency-Key") String idempotencyKey,HttpServletRequest servletRequest){actors.requireHuman(servletRequest,"ouf.managed-source.file.profile");var out=service.requestProfile(assetId,idempotencyKey);return ResponseEntity.accepted().location(URI.create("/api/onboarding/v1/managed-files/"+assetId+"/profile-jobs/"+out.get("job_id"))).body(out);}
