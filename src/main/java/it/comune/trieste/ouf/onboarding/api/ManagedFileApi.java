...
public class ManagedFileApi {
  ...
  @GetMapping("/{assetId}") public Map<String,Object> asset(@PathVariable UUID assetId,HttpServletRequest request){actors.requireHuman(request,"ouf.managed-source.preview");var row=service.asset(assetId);return Map.of("assetId",row.get("asset_id"),"status",row.get("status"),"mediaType",row.get("media_type"),"sizeBytes",row.get("size_bytes"));}
