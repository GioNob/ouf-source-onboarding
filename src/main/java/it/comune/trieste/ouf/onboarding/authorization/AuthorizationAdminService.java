...
public class AuthorizationAdminService {
 ...
 public List<Map<String,Object>> capabilities(int limit,int offset){if(limit<1||limit>200||offset<0||offset>100000)throw new IllegalArgumentException("limit 1..200; offset 0..100000");return db.sql("select capability_id,owner_ref,descriptor::text as descriptor from ouf_authorization.capability_registration order by capability_id limit :n offset :offset").param("n",limit).param("offset",offset).query().listOfRows();}
