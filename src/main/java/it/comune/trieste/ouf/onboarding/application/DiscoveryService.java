...
public class DiscoveryService {
  ...
  @Transactional public Map<String,Object> start(String sourceId,String bindingId,String idempotencyKey,Map<String,Object> options){
    Map<String,Object> binding=binding(sourceId,bindingId);String requestHash=hashes.of(Map.of("binding",binding,"options",options==null?Map.of():options));UUID run=UUID.randomUUID();
    db.sql("insert into ouf_onboarding.discovery_run(discovery_run_id,source_id,binding_id,idempotency_key,request_hash,status) values(:i,:s,:b,:k,:h,'READY') on conflict(source_id,idempotency_key) do nothing")
      .param("i",run).param("s",sourceId).param("b",bindingId).param("k",idempotencyKey).param("h",requestHash).update();
    Map<String,Object> found=db.sql("select discovery_run_id,source_id,binding_id,idempotency_key,request_hash,status,attempts,max_attempts,claimed_by,lease_until,retry_at,snapshot::text snapshot,error_code,created_at,updated_at,completed_at from ouf_onboarding.discovery_run where source_id=:s and idempotency_key=:k for update").param("s",sourceId).param("k",idempotencyKey).query().singleRow();
  ...
  @Transactional public Map<String,Object> complete(UUID runId,String worker,Map<String,Object> snapshot){
    Map<String,Object> run=db.sql("select discovery_run_id,source_id,status,claimed_by from ouf_onboarding.discovery_run where discovery_run_id=:i for update").param("i",runId).query().singleRow();
