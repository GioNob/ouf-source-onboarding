...
public class MappingService {
  ...
  @Transactional public Map<String,Object> buildDraft(String sourceId,String typeCode,OnboardingService.Actor actor,String correlation){
    ...
    config.put("semanticMapping",Map.of("mappingId","sm-"+sourceId+"-"+typeCode,"sourceType",Map.of("sourceId",sourceId,"typeCode",typeCode),"targetClasses",List.of(Map.of("classIri",target)),"propertyMappings",propertyMappings,"semanticRefs",refs));
    config.put("bundle",Map.of("bundleId","bundle-"+sourceId+"-draft","bundleVersion","1.0.0","environment","draft","source",Map.of("sourceId",sourceId,"sourceKind",source.get("source_kind"),"acquisitionMode",source.get("acquisition_mode")),"createdFromOnboardingVersion","pending","effectiveFrom","1970-01-01T00:00:00Z","checksum","sha256:pending","status","APPROVED","changeRepresentationProfile",Map.of("mode","FULL_SNAPSHOT")));
